...
export interface SOSResponseData {
  ...
  matched_responder: ResponderCandidate | null;
  notified_contacts_count: number;
  created_at: string;
}

export const triggerSOS = async (latitude: number, longitude: number, triggerType = 'MANUAL_SOS'): Promise<SOSResponseData> => {
  try {
...
export const fetchNearbyResponders = async (latitude: number, longitude: number, radiusMeters = 10000): Promise<ResponderCandidate[]> => {
  try {
...
export const acceptEmergencyRequest = async (requestId: string, responderId: string | null = null) => {
  try {
...
export const fetchMedicalProfile = async (userId: string | null = null) => {
  try {
...
export const updateMedicalProfile = async (profileData: any, userId: string | null = null) => {
  try {
...
export const fetchResponderWebShieldView = async (victimId: string) => {
  try {
